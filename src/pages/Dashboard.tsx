  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="mb-16 animate-fade-in">
          ...
          <p className="font-inter text-xl text-gray-600">Chat Now</p>
        </div>
          ...
          <div className="bg-gray-50 rounded-3xl p-12 shadow-sm">
            <div className="max-w-4xl mx-auto">
              <div className="grid grid-cols-80 gap-1 mb-8">
                {Array.from({
                length: totalWeeks
              }, (_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-all duration-200 ${index < filledWeeks ? 'bg-gray-800' : 'bg-gray-200 hover:bg-gray-300'}`} />)}
              </div>
        ...
        animationDelay: '0.4s'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            return <Card key={tool.id} className="group bg-white border-0 rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer hover:scale-[1.02]" onClick={() => navigate(tool.route)} onMouseEnter={() => setHoveredCard(tool.id)} onMouseLeave={() => setHoveredCard(null)}>
                  <CardContent className="p-0">
                    ...
                    <div className={`transition-all duration-300 overflow-hidden ${hoveredCard === tool.id ? 'max-h-32 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                      <div className="pt-4 border-t border-gray-100">
                        {tool.features.map((feature, index) => <p key={index} className="font-inter text-sm text-gray-500 leading-relaxed">
                            {feature}
                          </p>)}
                      </div>
                    </div>
